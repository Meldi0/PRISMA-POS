...
export const Login: React.FC = () => {
  ...
  return <div className="auth-layout">
    ...
    <main className="auth-main"><div className="auth-form"><div className="auth-mobile-brand"><Brand /></div><div className="page-eyebrow">{challenge ? 'LANGKAH 2 DARI 2' : 'SELAMAT DATANG KEMBALI'}</div><h2>{challenge ? 'Verifikasi akun Anda' : 'Masuk ke ruang kerja'}</h2><p>{challenge ? method === 'totp' ? 'Masukkan kode authenticator atau salah satu kode pemulihan Anda.' : `Kode sekali pakai telah dikirim ke ${masked}. Berlaku 5 menit; periksa juga folder spam.` : 'Gunakan email akun dinas Anda. Verifikasi dua langkah membantu melindungi akses layanan.'}</p><form onSubmit={submit} className="form-stack">
      {!challenge ? <><Field label="Email akun dinas" required><input autoComplete="username" className="input" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="nama@posindonesia.co.id" required /></Field><Field label="Kata sandi" required><div className="relative"><input autoComplete="current-password" className="input pr-12" type={visible ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required placeholder="Masukkan kata sandi Anda" /><button type="button" className="absolute right-3 top-3 text-slate-500" aria-label={visible ? 'Sembunyikan kata sandi' : 'Tampilkan kata sandi'} onClick={() => setVisible(value => !value)}>{visible ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></Field><label className="flex gap-2 items-center text-xs text-slate-600"><input type="checkbox" checked={remember} onChange={e => setRemember(e.target.checked)} className="accent-teal-700 w-4 h-4" />Ingat saya selama 7 hari di perangkat pribadi ini</label></> : <Field label={method === 'totp' ? 'Kode authenticator / pemulihan' : 'Kode verifikasi 6 digit'} required><input autoFocus autoComplete="one-time-code" className="input text-center tracking-widest text-xl" inputMode={method === 'email' ? 'numeric' : 'text'} pattern={method === 'email' ? '[0-9]{6}' : undefined} maxLength={method === 'email' ? 6 : 20} value={code} onChange={e => setCode(e.target.value)} required placeholder={method === 'email' ? '000000' : 'Kode authenticator'} /></Field>}
      ...
      {!challenge && (
        <div className="pt-3 border-t border-slate-200 mt-2">
          <div className="text-[11px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Akun Resmi Administrator:</div>
          <button
            type="button"
            onClick={() => {
            ...
            className="w-full py-2 px-3 rounded-lg bg-teal-50 hover:bg-teal-100 border border-teal-200 text-xs font-semibold text-teal-800 transition-all flex items-center justify-between cursor-pointer"
          >
            <div className="flex items-center gap-2">
              <ShieldCheck size={16} className="text-teal-700" />
              <span>Admin Pusat (Super Admin)</span>
            </div>
            <span className="font-mono text-[11px] text-teal-700">admin@poso.local</span>
          </button>
        </div>
      )}
