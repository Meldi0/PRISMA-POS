...
export const Login: React.FC = () => {
  ...
  return <div className="auth-layout">
    <aside className="auth-aside"><Brand /><div className="auth-copy"><div className="page-eyebrow" style={{color:'#85d4c4'}}>PORTAL MITRA AGEN POS INDONESIA</div><h1>Kendala operasional tercatat.<br /><em>Penanganan terarah.</em></h1><p>Satu ruang terpadu untuk mitra Agen Pos melaporkan kendala transaksi & sistem, berkoordinasi dengan helpdesk, dan memantau penyelesaiannya.</p><div className="auth-flow"><div><span>01</span>Laporkan kendala transaksi atau operasional</div><div><span>02</span>Ikuti respons penanganan tim helpdesk</div><div><span>03</span>Dapatkan solusi tuntas yang terdokumentasi</div></div></div><p className="text-xs text-slate-300 flex gap-2 items-center"><ShieldCheck size={16} />Akun terverifikasi · Akses portal Agen Pos</p></aside>
    <main className="auth-main"><div className="auth-form"><div className="auth-mobile-brand"><Brand /></div><div className="page-eyebrow">{challenge ? 'LANGKAH 2 DARI 2' : 'SELAMAT DATANG KEMBALI'}</div><h2>{challenge ? 'Verifikasi akun Anda' : 'Masuk ke ruang kerja'}</h2><p>{challenge ? method === 'totp' ? 'Masukkan kode authenticator atau salah satu kode pemulihan Anda.' : `Kode sekali pakai telah dikirim ke ${masked}. Berlaku 5 menit; periksa juga folder spam.` : 'Gunakan alamat email terdaftar Anda. Verifikasi dua langkah membantu melindungi akses akun.'}</p><form onSubmit={submit} className="form-stack">
      {!challenge ? <><Field label="Alamat email" htmlFor="login-email" required><input id="login-email" autoComplete="username" className="input" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="nama@email.com" required /></Field><Field label="Kata sandi" htmlFor="login-password" required><div className="relative"><input id="login-password" autoComplete="current-password" className="input pr-12" type={visible ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required placeholder="Masukkan kata sandi Anda" /><button type="button" className="absolute right-3 top-3 text-slate-500 hover:text-slate-700" aria-label={visible ? 'Sembunyikan kata sandi' : 'Tampilkan kata sandi'} onClick={() => setVisible(value => !value)}>{visible ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></Field><label htmlFor="login-remember" className="flex gap-2 items-center text-xs text-slate-700 cursor-pointer select-none"><input id="login-remember" type="checkbox" checked={remember} onChange={e => setRemember(e.target.checked)} className="accent-teal-700 w-4 h-4 rounded cursor-pointer" />Ingat saya selama 7 hari di perangkat pribadi ini</label></> : <Field label={method === 'totp' ? 'Kode authenticator / pemulihan' : 'Kode verifikasi 6 digit'} htmlFor="login-code" required><input id="login-code" autoFocus autoComplete="one-time-code" className="input text-center tracking-widest text-xl" inputMode={method === 'email' ? 'numeric' : 'text'} pattern={method === 'email' ? '[0-9]{6}' : undefined} maxLength={method === 'email' ? 6 : 20} value={code} onChange={e => setCode(e.target.value)} required placeholder={method === 'email' ? '000000' : 'Kode authenticator'} /></Field>}
    ...
    </form><div className="auth-footer">{challenge ? 'Jangan bagikan kode verifikasi kepada siapa pun.' : <>Belum punya akun? <Link to="/register">Daftarkan akun Agen Pos</Link><p className="mt-3">Lupa kata sandi atau akses email? Hubungi administrator helpdesk untuk pemulihan akun setelah verifikasi identitas.</p></>}</div></div></main>
  </div>;
};
