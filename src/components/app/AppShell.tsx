...
import { ArrowUpRight, Bell, Building2, Inbox, LayoutDashboard, LogOut, Menu, Plus, Search, ShieldCheck, Users, X, ChartNoAxesCombined, Settings, History, UserCheck } from 'lucide-react';
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadApprovals = async () => {
      try {
        const res = await apiService.getApprovals({ status: 'PENDING' });
        if (alive && res.status === 'success' && Array.isArray(res.data)) {
    ...
    const onUpdate = () => void loadApprovals();
  ...
  const adminMenu = [
    ...
    { view: 'approvals', icon: UserCheck, label: 'Persetujuan akun', show: isAdmin, count: pendingApprovals },
  ...
  return <div className="app-frame">
    ...
    <aside className={`app-sidebar ${open ? 'is-open' : ''}`} aria-label="Navigasi utama">
      ...
      {adminMenu.some(item => item.show) && <><div className="sidebar-caption">KELOLA LAYANAN</div><nav>{adminMenu.filter(item => item.show).map(item => <Link key={item.view} to={`/dashboard?view=${item.view}`} onClick={() => setOpen(false)} className={`nav-item ${location.pathname === '/dashboard' && active === item.view ? 'active' : ''}`}><item.icon size={19} /><span className="flex-1">{item.label}</span>{Boolean(item.count) && <span className="nav-badge">{item.count}</span>}</Link>)}</nav></>}
